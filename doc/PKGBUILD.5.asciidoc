...
*options (array)*::
	...
	*sccache*;;
		Allow the use of sccache during `build()`. More useful in its negative
		form `!sccache` with select packages that have problems building
		with sccache. Cannot be used alongside `distcc` and `ccache`.
...
*agerequirement*::
	Provides the minimum user age required to grant permission to install the
	package. Useful for enforcing compliance to various laws worldwide.
